...
> {
  ...
  createHandler(handlerFn: NextWebhookApiHandler<TPayload, TExtras>): NextApiHandler {
    return async (req, res) => {
      logger.debug(`Handler for webhook ${this.name} called`);
      ...
      await processSaleorWebhook<TPayload>({
        ...
        .then(async (context) => {
          logger.debug("Incoming request validated. Call handlerFn");

          return handlerFn(req, res, { ...(this.extraContext ?? ({} as TExtras)), ...context });
        })
        .catch(async (e) => {
          logger.debug(`Unexpected error during processing the webhook ${this.name}`);
          ...
          logger.debug("Unexpected error: %O", e);
