...
import { getConfigurationForChannel, PaymentAppConfigurator } from "../payment-app-configuration/payment-app-configuration";
import { PaymentAppConfigEntryFullyConfigured, paymentAppFullyConfiguredEntrySchema } from "../payment-app-configuration/config-entry";
...
const fetchHyperswitchConfiguration = async (configurator: PaymentAppConfigurator, channelId: string): Promise<PaymentAppConfigEntryFullyConfigured> => { 
...
export const fetchHyperswitchPaymentResponseHashKey = async (configurator: PaymentAppConfigurator, channelId: string): Promise<string> => { 
  const appConfig = await configurator.getConfig();
  const appChannelConfig = getConfigurationForChannel(appConfig, channelId);
  if (appChannelConfig == null) {
    throw new ChannelNotConfigured("Please assign a channel for your configuration");
...
export const createHyperswitchClient = async ({
  configurator, channelId
}: {
  configurator: PaymentAppConfigurator, channelId: string
}) => {
